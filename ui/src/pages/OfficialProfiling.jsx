...
const COMPLETENESS_FIELDS = [
    'last_name', 'first_name', 'gender', 'date_of_birth', 'civil_status',
...
const OfficialProfiling = () => {
    ...
    const generateCSV = () => {
        ...
        try {
            const header = [
                'Last Name', 'First Name', 'Middle Name', 'Suffix', 'Gender', 'Date of Birth', 'Age', 'Civil Status',
            ...
            const row = [
                profile.last_name, profile.first_name, profile.middle_name, profile.suffix, profile.gender, profile.date_of_birth, profile.age, profile.civil_status,
                                            ...
                                            <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    {prevPositions.map((pos, idx) => (
                                                        <motion.div key={pos.position_id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_130px_130px_70px_44px] gap-3 items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                                            <input type="text" value={pos.position_name || ''} onChange={e => setPrevPositions(p => p.map((x, i) => i === idx ? { ...x, position_name: e.target.value } : x))} placeholder="Position" className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-[11px] font-bold outline-none focus:border-[#0038A8] transition-all truncate min-w-0" />
                                                            ...
                                                            <div className="relative">
                                                                <input type="date" value={pos.start_date ? pos.start_date.split('T')[0] : ''} onChange={e => setPrevPositions(p => p.map((x, i) => i === idx ? { ...x, start_date: e.target.value } : x))} className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-[10px] font-bold outline-none focus:border-[#0038A8] transition-all w-full pr-8" />
                                                                <FiCalendar
                                            ...
                                            <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-6">
                                                ...
                                                <Field label="Awards / Recognitions / Notable Achievements (No Abbreviations)">
                                                    <textarea
                                                        ...
                                                        className="w-full bg-slate-50 border-2 border-transparent focus:border-[#FCD116] rounded-[2rem] py-5 px-6 text-sm font-bold outline-none transition-all resize-none"
                                                    />
                                                </Field>
                                            </div>
                                        </div>
                                            ...
                                            <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-6">
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    {trainings.map((tr, idx) => (
                                                        <motion.div key={tr.training_id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_160px_100px_44px] gap-3 items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                                            <input type="text" value={tr.training_name || ''} onChange={e => setTrainings(t => t.map((x, i) => i === idx ? { ...x, training_name: e.target.value } : x))} placeholder="Training / Seminar name" className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:border-[#0038A8] transition-all min-w-0" />
                                                            <div className="relative">
                                                                <input type="date" value={tr.date_completed ? tr.date_completed.split('T')[0] : ''} onChange={e => setTrainings(t => t.map((x, i) => i === idx ? { ...x, date_completed: e.target.value } : x))} className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:border-[#0038A8] transition-all min-w-0 w-full pr-8" />
                                                                <FiCalendar
                                                                    className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-200 cursor-pointer hover:text-[#0038A8] transition-colors"
                                                                    size={12}
                                                                    onClick={(e) => e.currentTarget.previousSibling.showPicker()}
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                                                {[
                                                    ...
                                                    <div key={id} className="flex flex-col gap-3 p-6 bg-white rounded-[2rem] border border-slate-100 hover:border-[#0038A8]/20 transition-all">
                                                        <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="relative group/upload">
                                                            ...
                                                            />
                                                            <div className={`flex items-center justify-center gap-3 border-2 border-dashed rounded-xl px-4 py-3 transition-all ${profile[`${id}_binary_id`] ? 'bg-emerald-50/30 border-emerald-100 text-emerald-600' : 'bg-slate-50 border-slate-200 text-slate-400 group-hover/upload:border-[#0038A8] group-hover/upload:text-[#0038A8]'}`}>
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-6">
                                                <Field label="Pending Administrative Case/s (Type 'Not Applicable' if none)">
                                                    <textarea value={profile.pending_admin_case} onChange={e => setP('pending_admin_case', e.target.value)} rows={4} placeholder="Type 'Not Applicable' if none. Otherwise, describe the nature and status of the case." className="w-full bg-slate-50 border-2 border-transparent focus:border-amber-400 rounded-2xl py-3 px-5 text-sm font-bold outline-none transition-all resize-none" />
                                                </Field>
                                                                            ...
                                                                            <div className="w-full bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                                                                                ...
                                                                                <div className="p-0 overflow-x-auto">
                                                                                    <table className="w-full text-left border-collapse text-xs font-mono whitespace-nowrap">
                                                                                        ...
                                                                                        <tbody>
                                                                                            {[
                                                                                                ['Last Name', profile.last_name], ['First Name', profile.first_name], ['Middle Name', profile.middle_name],
                            ...
                            {applicationStatus === 'disapproved' && tab === 'application' && (
                                <div className="mt-10 flex justify-center">
                                    <button
                                        ...
                                        disabled={saving}
                                        className="px-12 py-5 bg-[#CE1126] text-white font-black text-[10px] uppercase tracking-widest rounded-full shadow-2xl hover:bg-red-800 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-4"
                                    >
